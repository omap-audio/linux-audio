...
static inline struct tc358768_priv *dsi_host_to_tc358768(
						struct mipi_dsi_host *host)
{
	return container_of(host, struct tc358768_priv, dsi_host);
}

static inline struct tc358768_priv *bridge_to_tc358768(
						struct drm_bridge *bridge)
{
...
static void tc358768_bridge_enable(struct drm_bridge *bridge)
{
	...
	case MIPI_DSI_FMT_RGB565:
		...
		break;
	default:
		dev_err(priv->dev, "Invaid data format (%u)\n",
			dsi_dev->format);
		return;
	}
...
MODULE_DESCRIPTION("TC358768AXBG/TC358778XBG RBG to DSI bridge");
