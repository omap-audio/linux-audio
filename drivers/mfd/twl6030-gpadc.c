...
struct twl6025_chnl_calib {
	s32 gain;
	s32 gain_error;
	s32 offset_error;
};

struct twl6025_ideal_code {
	s16 code1;
	s16 code2;
	s16 v1;
	s16 v2;
...
#define TWL6025_GPADC_TRIM1	0xCD
#define TWL6025_GPADC_TRIM2	0xCE
#define TWL6025_GPADC_TRIM3	0xCF
#define TWL6025_GPADC_TRIM4	0xD0
#define TWL6025_GPADC_TRIM5	0xD1
#define TWL6025_GPADC_TRIM6	0xD2
#define TWL6025_GPADC_TRIM7	0xD3
#define TWL6025_GPADC_TRIM8	0xD4
#define TWL6025_GPADC_TRIM9	0xD5
#define TWL6025_GPADC_TRIM10	0xD6
#define TWL6025_GPADC_TRIM11	0xD7
#define TWL6025_GPADC_TRIM12	0xD8
#define TWL6025_GPADC_TRIM13	0xD9
#define TWL6025_GPADC_TRIM14	0xDA
#define TWL6025_GPADC_TRIM15	0xDB
#define TWL6025_GPADC_TRIM16	0xDC
#define TWL6025_GPADC_TRIM19	0xFD
...
static const struct twl6025_ideal_code
			twl6025_ideal[TWL6025_GPADC_MAX_CHANNELS] = {
	{	/* CHANNEL 0 */
		.code1 = 1441,
		.code2 = 3276,
		.v1 = 440,
...
struct twl6030_gpadc_data {
	...
	int irq_n;
	struct twl6025_chnl_calib *twl6025_cal_tbl;
...
static const
struct twl6030_gpadc_conversion_method twl6025_conversion_methods_table[] = {
	[TWL6030_GPADC_RT] = {
		.sel	= TWL6025_GPADC_RTSELECT_LSB,
		.rbase	= TWL6025_RTCH0_LSB,
		.mask	= TWL6025_GPADC_RT_EOC_MASK,
	},
	[TWL6030_GPADC_SW2] = {
		.sel	= TWL6025_GPADC_GPSELECT_ISB,
		.rbase	= TWL6025_GPCH0_LSB,
		.ctrl	= TWL6025_GPADC_CTRL_P1,
		.enable = TWL6030_GPADC_CTRL_P1_SP1,
		.mask	= TWL6025_GPADC_SW_EOC_MASK,
	},
};
	...
	struct device_attribute *devattr, const char *buf, size_t count)
{
	...
	if ((kstrtol(buf, 10, &val) < 0) || (val < 15000) || (val > 60000))
		return -EINVAL;
		...
		u8 reg_base, u32 channels, struct twl6030_gpadc_request *req)
{
	...
	channels = ~channels;
	if (gpadc->features & TWL6025_SUBCLASS) {
		for (i = 0; i < TWL6025_GPADC_MAX_CHANNELS; i++) {
			if (channels & BIT(i))
				continue;
			...
			if (unlikely((i >= 15) && (i <= 17))) {
				...
				((raw_code * 1000) -
				gpadc->twl6025_cal_tbl[i].offset_error) /
				gpadc->twl6025_cal_tbl[i].gain_error;
...
static void twl6030_gpadc_work(struct work_struct *ws)
{
	...
	for (i = 0; i < TWL6030_GPADC_NUM_METHODS; i++) {
		...
		if (r->channels & (1 << 2))
			twl6030_gpadc_write(the_gpadc, TWL6030_GPADC_CTRL, 0x0);
	...
	const struct twl6030_gpadc_conversion_method *method)
{
	...
	if (req->channels & (1 << 2))
		twl6030_gpadc_write(the_gpadc, TWL6030_GPADC_CTRL, 0x4);
...
/* locks held by caller */
static int _twl6025_gpadc_conversion(struct twl6030_gpadc_request *req,
	const struct twl6030_gpadc_conversion_method *method)
{
	...
	if (req->method == TWL6030_GPADC_RT) {
		/*
		 * For the TWL6025 real time conversion
	...
	/*
	 * For the TWL6025 Asynchronous Conversion
...
int twl6030_gpadc_conversion(struct twl6030_gpadc_request *req)
{
	...
	if (the_gpadc->features & TWL6025_SUBCLASS)
		ret = _twl6025_gpadc_conversion(req, method);
	else
			       ...
			       unsigned long arg)
{
	...
	case TWL6030_GPADC_IOCX_ADC_RAW_READ: {
		...
		int max_channels = (the_gpadc->features & TWL6025_SUBCLASS) ?
			TWL6025_GPADC_MAX_CHANNELS : TWL6030_GPADC_MAX_CHANNELS;
...
static int twl6025_calibration(struct twl6030_gpadc_data *gpadc)
{
	 ...
	 */
	for (chn = 0; chn < TWL6025_GPADC_MAX_CHANNELS; chn++) {
		...
		/* Gain */
		gain = ((twl6025_ideal[chn].v2 -
			twl6025_ideal[chn].v1) * 1000) /
			((twl6025_ideal[chn].code2 -
			twl6025_ideal[chn].code1));

		x1 = twl6025_ideal[chn].code1;
		x2 = twl6025_ideal[chn].code2;
		...
		gpadc->twl6025_cal_tbl[chn].gain = gain;
		gpadc->twl6025_cal_tbl[chn].gain_error = k;
		gpadc->twl6025_cal_tbl[chn].offset_error = b;
...
static int __devinit twl6030_gpadc_probe(struct platform_device *pdev)
{
	...
	gpadc = kzalloc(sizeof(struct twl6030_gpadc_data), GFP_KERNEL);
	if (!gpadc)
		return -ENOMEM;
	...
	if (pdata->features & TWL6025_SUBCLASS) {
		gpadc->twl6025_cal_tbl = kzalloc(
				sizeof(struct twl6025_chnl_calib) *
				TWL6025_GPADC_MAX_CHANNELS,
				GFP_KERNEL);
		if (!gpadc->twl6025_cal_tbl) {
			ret = -ENOMEM;
			goto err_pdata;
		}
	}
	dev_err(&pdev->dev, "Here 04\n");
	...
	if (gpadc->features & TWL6025_SUBCLASS)
		twl6030_conversion_methods = twl6025_conversion_methods_table;
	...
	ret = request_threaded_irq(irq_rt, NULL, twl6030_gpadc_irq_handler,
		IRQF_ONESHOT, "twl6030_gpadc_rt", &gpadc->requests[TWL6030_GPADC_RT]);
	if (ret) {
	...
	ret = request_threaded_irq(irq, NULL, twl6030_gpadc_irq_handler,
		IRQF_ONESHOT, "twl6030_gpadc_sw", &gpadc->requests[TWL6030_GPADC_SW2]);
	if (ret) {
		dev_err(&pdev->dev, "could not request irq\n");
		goto err_irq_rt;
	}
