...
static void abe_free_fw(struct omap_abe *abe)
{
	...
	/* This should be done in HAL  - oposite of init_mem()*/
	if (abe->aess) {
		if (abe->aess->fw_info) {
...
static int abe_control_load(struct snd_soc_platform *platform,
...
static int abe_widget_load(struct snd_soc_platform *platform,
...
static int abe_vendor_unload(struct snd_soc_platform *platform,
	struct snd_soc_fw_hdr *hdr)
{
	return 0;
}
