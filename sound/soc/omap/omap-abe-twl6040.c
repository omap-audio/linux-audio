...
static void omap_abe_fw_ready(const struct firmware *fw, void *context)
{
	...
	if (card_data->has_abe) {
		ret = omap_abe_add_dai_links(card, NULL);
		if (ret < 0)
			goto err_unregister;
	}

	ret = omap_abe_add_legacy_dai_links(card, NULL);
	if (ret < 0)
		goto err_unregister;
...
static int omap_abe_probe(struct platform_device *pdev)
{
	...
	if (node) {
		...
		dai_node = of_parse_phandle(node, "ti,mcpdm", 0);
		if (!dai_node) {
			dev_err(&pdev->dev, "McPDM node is missing\n");
			return -EINVAL;
		}
		...
		if (dai_node) {
			pdev_tmp = platform_device_register_simple("dmic-codec",
								   -1, NULL, 0);
			if (IS_ERR(pdev_tmp)) {
				dev_err(&pdev->dev,
					"Can't instantiate dmic-codec\n");
				return PTR_ERR(pdev_tmp);
			}
			card_data->dmic_codec_dev = pdev_tmp;
