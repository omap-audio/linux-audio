				       ...
				       bool power, bool connect)
{
	...
	if (power != peer->power || peer->id == snd_soc_dapm_dai_in ||
	    peer->id == snd_soc_dapm_dai_out)
				  ...
				  struct list_head *down_list)
{
	...
	if (w->power == power &&
	    (w->id != snd_soc_dapm_dai_in || w->id != snd_soc_dapm_dai_out))
		return;
	...
	int event)
{
	...
	dapm_power_widgets(dai->card, event);
	w->dai_endpoint = 0;
}
